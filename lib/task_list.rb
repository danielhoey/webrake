...
class TaskListTest < Minitest::Unit::TestCase
  ...
  def test_rule_output_is_input_to_another_rules
    ...
    task_list = TaskList.new(RuleList.new({
      Glob.new('*.erb') => Rule.new(:unused, :unused, ''),
      Glob.new('*.less') => Rule.new(:unused, :unused, '')}),
  ...
  def test_endless_loop
    ...
    task_list = TaskList.new(RuleList.new({Glob.new('*.html') => Rule.new(:unused, :unused, '')}), input_files)
